...
#define CREATE_UNARY_OPERATOR(graphBuilder, type, inputTensorDesc) \
    ...
    DML_TENSOR_DESC Graph::AppendIdentity(const DML_TENSOR_DESC& inputTensorDesc) {
        DML_TENSOR_DESC outputTensorDesc = CreateTensorDesc(&inputTensorDesc, {}, {}, true);
    ...
    DML_TENSOR_DESC Graph::CreateTensorDesc(const std::vector<UINT>& dimensions,
        ...
        mDmlTensorsDesc.push_back(dmlTensorDesc);
        return {DML_TENSOR_TYPE_BUFFER, &dmlTensorDesc->bufferDesc};
    }

    DML_TENSOR_DESC Graph::CreateTensorDesc(OperandDescriptor const* desc,
                                            DML_TENSOR_FLAGS tensorFlag) {
        ...
        return CreateTensorDesc(dimensions, {}, dataType, tensorFlag);
    }

    DML_TENSOR_DESC Graph::CreateTensorDesc(DML_TENSOR_DESC const* tensorDesc,
                                            std::vector<UINT> dimensions,
        ...
        return CreateTensorDesc(dimensions, strides, desc->DataType, tensorFlags);
    }
    ...
    MaybeError Graph::TransposeOutputToNhwc(std::shared_ptr<NodeBase>& inputNode,
        ...
        inputNode =
            updateNode(inputNode, CreateTensorDesc(&inputNode->outputTensorDesc, nhwcOutputDims,
                                                   nhwcOutputStrides, true));
        return {};
    }

    Graph::Graph(Context* context) : GraphBase(context), mDevice(new Device()) {
        ...
        // Set up Direct3D 12
        mDevice->Init(gpuPreference, useGpu);
    ...
    MaybeError Graph::AddConstant(const op::Constant* constant) {
        ...
        inputNode->outputTensorDesc = CreateTensorDesc(desc, DML_TENSOR_FLAG_OWNED_BY_DML);
    ...
    std::shared_ptr<InputNode> Graph::CreateConstantInput(void const* value,
                                                          size_t size,
        ...
        DML_TENSOR_DESC inputTensorDesc =
            CreateTensorDesc(dimensions, strides, dataType, tensorFlag);
    ...
    MaybeError Graph::AddInput(const op::Input* input) {
        ...
        std::shared_ptr<InputNode> inputNode(new InputNode());
        inputNode->outputTensorDesc = CreateTensorDesc(desc);
        inputNode->name = input->GetName();
        inputNode->type = NodeType::NonConstantInput;
        inputNode->inputIndex = mInputs.size();
        inputNode->byteLength = mDmlTensorsDesc.back()->bufferDesc.TotalTensorSizeInBytes;
        std::shared_ptr<NodeBase> nodeBase(inputNode);
    ...
    MaybeError Graph::AddBinary(const op::Binary* binary) {
        ...
        DML_TENSOR_DESC aTensorDesc =
            CreateTensorDesc(&aNode->outputTensorDesc, aNewDims, aNewStrides);
        auto bNewStrides = CalculateStridesForBroadcast(bDims, bNewDims, bNode->outputTensorDesc,
                                                        broadcastSkipAxis);
        DML_TENSOR_DESC bTensorDesc =
            CreateTensorDesc(&bNode->outputTensorDesc, bNewDims, bNewStrides);
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&aNode->outputTensorDesc, outputNewDims, {}, true);
        ...
        if (outputDims != outputNewDims) {
            outputTensorDesc = CreateTensorDesc(&aNode->outputTensorDesc, outputDims, {}, true);
        }
    ...
    MaybeError Graph::HardSwish(std::shared_ptr<NodeBase>& inputNode,
        ...
        std::shared_ptr<NodeBase> intermediateNode, secondConstantInputNode, outputNode;
        uint32_t length = SizeOfShape(inputDims);
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&inputNode->outputTensorDesc, inputDims, {}, true);

        std::vector<float> constant(length, 3);
        // x+3
        {
            // Create the first constant input.
            auto firstConstantInputNode =
                CreateConstantInput(constant.data(), length * sizeof(float), inputDims, {},
        ...
        switch (unary->GetType()) {
            ...
            case op::UnaryOpType::kNeg: {
                ...
                if (inputOperand->Type() == wnn::OperandType::Float32) {
                    ...
                    constantInputNode =
                        CreateConstantInput(constant.data(), length * sizeof(int32_t), inputDims,
    ...
    MaybeError Graph::AddSplit(const op::Split* split) {
        ...
        for (size_t i = 0; i < outputNum; ++i) {
            DML_TENSOR_DESC outputTensorDesc =
                CreateTensorDesc(&inputNode->outputTensorDesc,
    ...
    MaybeError Graph::AddReshape(const op::Reshape* reshape) {
        ...
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&inputNode->outputTensorDesc, outputDims);
    ...
    MaybeError Graph::AddTranspose(const op::Transpose* transpose) {
        ...
        mGraphNodesMap[transpose->PrimaryOutput()] = updateNode(
            inputNode,
            CreateTensorDesc(&inputNode->outputTensorDesc, outputDims, transposedStrides));
        return {};
    }
    ...
    MaybeError Graph::AddConv2d(const op::Conv2d* conv2d) {
        ...
        if (options->inputLayout == wnn::InputOperandLayout::Nhwc) {
            ...
            inputTensorDesc =
                CreateTensorDesc(&inputNode->outputTensorDesc, newInputDims, newInputStrides);
        }
        ...
        if (options->filterLayout != wnn::Conv2dFilterOperandLayout::Oihw) {
            ...
            filterTensorDesc =
                CreateTensorDesc(&filterNode->outputTensorDesc, newFilterDims, newFilterStrides);
        }
        ...
        if (options->bias != nullptr) {
            ...
            newBiasTensorDesc = CreateTensorDesc(&biasNode->outputTensorDesc, newBiasDims);
            biasTensorDescPtr = &newBiasTensorDesc;
            inputNodes.push_back(biasNode);
        }
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&inputNode->outputTensorDesc, newOutputDims, {}, true);
    ...
    MaybeError Graph::AddPool2d(const op::Pool2d* pool2d) {
        ...
        if (pool2d->GetType() == op::Pool2dType::kAveragePool2d) {
            ...
            if (dilations[0] != 1 || dilations[1] != 1) {
                ...
                outputTensorDesc = CreateTensorDesc(newOutputDims);
            }
    ...
    MaybeError Graph::AddPad(const op::Pad* pad) {
        ...
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&paddingTensorDesc, paddingDims, {}, true);
        ...
        DML_OUTPUT_GRAPH_EDGE_DESC outputNodeDesc;
        auto outputNodePtr = reinterpret_cast<Node*>(outputNode.get());
        outputNodeDesc.FromNodeIndex = outputNodePtr->nodeIndex;
        outputNodeDesc.FromNodeOutputIndex = outputNodePtr->outputNodeIndex;
        outputNodeDesc.GraphOutputIndex = mOutputs.size();
        mGraphBuilder->AddOutputEdge(outputNodeDesc);
        mOutputs.push_back(*outputNodePtr);
        ...
        outputTensorDesc = CreateTensorDesc(&inputNode->outputTensorDesc, outputDims, {}, true);
    ...
    MaybeError Graph::AddBatchNorm(const op::BatchNorm* batchNorm) {
        ...
        for (size_t i = 1; i < inputs.size(); ++i) {
            ...
            DML_TENSOR_DESC tensorDesc = CreateTensorDesc(&node->outputTensorDesc, expandDims);
        ...
        if (options->scale == nullptr) {
            ...
            auto constantInputNode = CreateConstantInput(&scale, sizeof(float), {1, 1, 1, 1}, {},
                                                         DML_TENSOR_DATA_TYPE_FLOAT32);
            tensorsDesc.insert(
        ...
        if (options->bias == nullptr) {
            ...
            // Create a constant scale.
            auto constantInputNode = CreateConstantInput(&bias, sizeof(float), {1, 1, 1, 1}, {},
                                                         DML_TENSOR_DATA_TYPE_FLOAT32);
    ...
    MaybeError Graph::AddGru(const op::Gru* gru) {
        ...
        DML_TENSOR_DESC inputTensorDesc = CreateTensorDesc(&inputNode->outputTensorDesc, inputDims);
        ...
        DML_TENSOR_DESC weightTensorDesc =
            AppendIdentity(CreateTensorDesc(&constantWeightNode->outputTensorDesc, weightDims));
        ...
        DML_TENSOR_DESC recurrenceTensorDesc = AppendIdentity(
            CreateTensorDesc(&constantRecurrenceNode->outputTensorDesc, recurrenceDims));
        ...
        if (options->bias != nullptr) {
            ...
            webnnBiasNode =
                CreateConstantInput(biasConstantData.data(), webnnBiasLength * sizeof(float),
                                    webnnBiasDims, {}, DML_TENSOR_DATA_TYPE_FLOAT32);
        }
        ...
        DML_TENSOR_DESC joinOutputTensorDesc = CreateTensorDesc(
            &webnnBiasNode->outputTensorDesc, {webnnBiasDims[0], webnnBiasDims[1] * 2}, {}, true);
        ...
        DML_TENSOR_DESC biasTensorDesc = CreateTensorDesc(&biasNode->outputTensorDesc, biasDims);
        inputNodes.push_back(biasNode);
        ...
        if (options->initialHiddenState != nullptr) {
            ...
            hiddenInitTensorDesc = AppendIdentity(
                CreateTensorDesc(&constantHiddenInitNode->outputTensorDesc, hiddenInitDims));
            ...
            inputNodes.push_back(mGraphBuilder->CreateNode(hiddenInitTensorDesc));
        }
        ...
        if (options->returnSequence) {
            ...
            outputSequenceTensorDesc = CreateTensorDesc(outputSequenceSizes);
        ...
        DML_TENSOR_DESC outputSingleTensorDesc = CreateTensorDesc(outputSingleSizes);
        ...
        DML_TENSOR_DESC webnnOutputSingleTensorDesc =
            CreateTensorDesc(&outputSingleNode->outputTensorDesc, webnnOutputSingleDims);
...
#define CREATE_REDUCE_OPERATOR(graphBuilder, type, inputTensorDesc, outputTensorDesc, axes) \
    ...
    MaybeError Graph::AddReduce(const op::Reduce* reduce) {
        ...
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&inputTensorDesc, reducedDims, {}, true);
        ...
        if (!options->keepDimensions) {
            outputTensorDesc = CreateTensorDesc(&outputTensorDesc, outputDims);
        }
        mGraphBuilder->AddNodes({inputNode});
    ...
    MaybeError Graph::AddResample2d(const op::Resample2d* resample2d) {
        ...
        auto inputTensorDesc = inputNode->outputTensorDesc;
        DML_TENSOR_DESC outputTensorDesc = CreateTensorDesc(&inputTensorDesc, outputDims);
...
#define SLICE_ONE_AXIS(axis, index)                                                       \
    ...
    MaybeError Graph::AddSlice(const op::Slice* slice) {
        ...
        auto inputTensorDesc = inputNode->outputTensorDesc;
        DML_TENSOR_DESC outputTensorDesc = CreateTensorDesc(&inputTensorDesc, outputDims, {}, true);
    ...
    MaybeError Graph::AddInstanceNorm(const op::InstanceNorm* instanceNorm) {
        ...
        if (options->scale == nullptr) {
            ...
            std::vector<UINT> scaleDims = {1, newInputDims[1], 1, 1};
            // Create a constant scale.
            auto constantInputNode =
                CreateConstantInput(scale.data(), newInputDims[1] * sizeof(float), scaleDims, {},
                                    DML_TENSOR_DATA_TYPE_FLOAT32);
        ...
        if (options->bias == nullptr) {
            ...
            // Create a constant scale.
            auto constantInputNode =
                CreateConstantInput(bias.data(), newInputDims[1] * sizeof(float), biasDims, {},
                                    DML_TENSOR_DATA_TYPE_FLOAT32);
            tensorsDesc.push_back(constantInputNode->outputTensorDesc);
            inputNodes.push_back(constantInputNode);
        }
    ...
    MaybeError Graph::AddConcat(const op::Concat* concat) {
        ...
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&primaryNode->outputTensorDesc, newOutputDims, {}, true);
        ...
        if (outputDims.size() < newOutputDims.size()) {
            outputTensorDesc = CreateTensorDesc(&primaryNode->outputTensorDesc, outputDims);
        }
    ...
    MaybeError Graph::AddGemm(const op::Gemm* gemm) {
        ...
        aDims = ExpandDimensions(aDims, 4);
        DML_TENSOR_DESC aTensorDesc = CreateTensorDesc(&aNode->outputTensorDesc, aDims);

        DAWN_ASSERT(bDims.size() == 2);
        bDims = ExpandDimensions(bDims, 4);
        DML_TENSOR_DESC bTensorDesc = CreateTensorDesc(&bNode->outputTensorDesc, bDims);

        DAWN_ASSERT(outputDims.size() == 2);
        auto expandedOutputDims = ExpandDimensions(outputDims, 4);
        DML_TENSOR_DESC outputTensorDesc =
            CreateTensorDesc(&aNode->outputTensorDesc, expandedOutputDims, {}, true);
        ...
        if (inputsOperand.size() == 3) {
            ...
            cTensorDesc = CreateTensorDesc(&cNode->outputTensorDesc, cNewDims, cNewStrides);
    ...
    MaybeError Graph::AddOutput(std::string_view name, const OperandBase* output) {
        ...
        if (outputNode->type == NodeType::ConstantInput ||
            ...
            DML_TENSOR_DESC outputTensorDesc = AppendIdentity(outputNode->outputTensorDesc);
    ...
    MaybeError Graph::CompileImpl() {
        ...
        // GPU.
        mCompiledGraph.reset(new CompiledGraph(mDevice->GetIDMLDevice(), graphDesc, mInputs,
                                               mOutputs, DML_EXECUTION_FLAG_NONE));
        mGraphBuilder.reset(nullptr);
        mDevice->InitializeGraph(mCompiledGraph.get());
        return {};
    }
    ...
    MaybeError Graph::ComputeImpl(NamedInputsBase* inputs, NamedOutputsBase* outputs) {
        ...
        mDevice->ExecuteGraph(mCompiledGraph.get(), namedInputs, outputs);
        return {};
    }
}  // namespace webnn::native::dml
